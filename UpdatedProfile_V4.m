...
Racrit=1708; %for constant viscosity fluid
%^from 'Scaling of temperature‐ and stress‐dependent viscosity convection'
...
sgtitle('Initial Adiabatic Conditions') 
subplot(2,3,1)
...
%     if rem(i,100)==0
        ...
        [cb,Tm] = findmeltT(phasenew',0,Pressure);
        test2(:,ss)  = phasenew';
%     end
    ...
    [Ra,Nu,td] = RayleighNumberCalculator(T_Start,T_surf,Base_Flux,g,Rx,k_i,rho_i,c_i,alpha_i,Pressure,Height_list);
...
cb = colorbar
ylabel(cb,'Temperature (K)','FontSize',24)
xlabel('Time (Myr)')
ylabel('Depth (km)')
set(gca,'FontSize',24)
set(gca,'LineWidth',2,'TickLength',[0.03 0.03]);
...
xlabel('Time (Myr)')
ylabel('Depth (km)')
ylabel(cb,'Rayleigh Number','FontSize',24)
set(gca,'FontSize',24)
set(gca,'LineWidth',2,'TickLength',[0.03 0.03]);
colormap spring

ax(2) = subplot(2,1,2)
imagesc([0:timenew:Timescale]./1E6,[Height_list],([test4]))
cb=colorbar
xlabel('Time (Myr)')
ylabel('Depth (km)')
set(gca,'FontSize',24)
set(gca,'LineWidth',2,'TickLength',[0.03 0.03]);
ylabel(cb,'Nusselt Number','FontSize',24)
colormap spring
...
%Rayleigh Calculator Initial Plots
    
figure('units','normalized','position',[.1 .1 .5 .6])
sgtitle('Initial Adiabatic Conditions') 
subplot(2,1,1)
plot(test3(:,1),Height_list,'k','LineWidth',1.9);
xlabel('Rayleigh Number');
ylabel('Depth(km)');
hold on
xline(Racrit,'--','Critical Rayleigh Threshold');
set(gca,'FontSize',20);
set(gca,'LineWidth',2,'TickLength',[0.03 0.03]);
%t = text(xmean,ymean,'Critical Rayleigh','FontSize',16,'Color','b')

subplot(2,1,2)
plot(test4(:,1),Height_list,'k','LineWidth',1.9);
