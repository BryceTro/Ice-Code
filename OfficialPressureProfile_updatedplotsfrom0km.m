...
%also have to input same heights into seafreezecall

T_surf=235; %Surface Temp of planet (in K)
Base_Flux=60/1000;  %Heat Flux (in W/m^2)
...
h1=14.804; %height of glacier up to Ice I and Ice II boundary
h2=22.0166; %height of glacier between Ice II and Ice V boundary
h3=42.5991; %height of glacier between Ice V and Ice VI boundary
h4=81.2502; %height of glacier between Ice VI and Ice VII boundary
...
elseif (h<14.804);
    ...
    P1=Patmosphere +(rho1*g*(h-1))*1000; %pressure of glacier up to Ice I and Ice II boundary
 
elseif (h>=14.804) && (h<22.0166);
    
    P1=Patmosphere +((rho1*g*h1) + (rho2*g*(h-h1-1)))*1000; %pressure of glacier between Ice II and Ice V boundary
    
elseif (h>=22.0166) && (h<42.5991);
    
    P1=Patmosphere +((rho1*g*(h1-1)) + (rho2*g*(h2-h1-1))+(rho5*g*(h-h2-1)))*1000; %pressure of glacier between Ice V and Ice VI boundary
    
else %(h>=42.5991) && (h<81.2502);
    
    P1=Patmosphere +((rho1*g*(h1-1)) + (rho2*g*(h2-h1-1))+(rho5*g*(h3-h2-1)+(rho1*g*(h-h3-1))))*1000; %pressure of glacier between Ice VI and Ice VII boundary
