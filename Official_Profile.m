...
Gibbs_Energy_list(i)=Gibbs_Energy;
...
%Equilibrium=SF_PhaseLines('Ih', 'water1') 
...
Thermal_Conductivity_list,Melting_Temperature_list,Gibbs_Energy_list);
 
 
Pressure_Profile_Final = array2table(Pressure_Profile,'RowNames',{'Height(km)','Pressure(MPa)',...
    'Phase','Density(kg/m^3)','Specific Heat((J/kg/K)','Thermal Conductivity(W/m/K)','Melting Temperature(K)','Gibbs Energy(J/kg)'})
...
hold on
subplot(2,4,1)
plot(Gibbs_Energy_list,-Height_list,'LineWidth',1.9)
set(gca,'FontSize',12)
set(gca,'LineWidth',2,'TickLength',[0.03 0.03]);
...
for time=[dt:dt:1000*dt]  
  ...
  if rem(i,100)==0
      ss = ss+1;
           [G,k_i,rho_i,c_i,Tm,Evolve_Phase_list]=SeaFreezeCall(T_Start,Pressure_list);
...
figure
imagesc([(dt:dt:1000*dt)/3.15576e+13],[Height_list],([test]))
cb=colorbar
ylabel('Depth(km)')
xlabel('Years(Ma)')
title(cb,'Temperature(K)')

figure
imagesc([(dt:dt:1000*dt)/3.15576e+13],[Height_list],([test2]))
cb=colorbar
ylabel('Depth(km)')
xlabel('Years(Ma)')
title(cb,'Ice Phase')
...
ylim([200 500])
title('Temperature Profile Final at Time=T')
...
title('Equilibrium Phase of Ice Ih-Water')

subplot(2,2,2)
SF_PhaseLines('III', 'water1','plot', 'meta')
title('Equilibrium Phase of Ice III-Water')

subplot(2,2,3)
SF_PhaseLines('V', 'water1','plot', 'meta')
title('Equilibrium Phase of Ice V-Water')

subplot(2,2,4)
SF_PhaseLines('VI', 'water1','plot', 'meta')
title('Equilibrium Phase of Ice VI-Water')
