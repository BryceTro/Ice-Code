...
dt=86400*365.25*1000;   %time step (s) of 1000 years
...
% plots over time
subplot(2,2,1)
plot(T_Start,-[Height_list])
hold on
i=1;
while i<size(hold_temps,2)
    plot(hold_temps(:,i),-[Height_list])
    i=i+1;
end
xlabel('Temp (K)')
ylabel('Height (km)')
...
subplot(2,2,2)
plot(Phi_Start,-[Height_list])
hold on
i=1;
while i<size(hold_phi,2)
    plot(hold_phi(:,i),-[Height_list])
    i=i+1;
end
xlabel('Porosity')
ylabel('Height (km)')
...
% final profile
subplot(2,2,3)
plot(T_new,-[Height_list])
xlabel('Temp (K)')
ylabel('Height (km)')

subplot(2,2,4)
plot(Phi_new,-[Height_list])
xlabel('Porosity')
ylabel('Height (km)')
...
plot((-[Height_list]), [Melting_Temperature_list])
%xlim([0 2.3e9])  
xlabel('Height( km)')
