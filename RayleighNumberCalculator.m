...
gamma(skip)= (tau(i)*h(i))/shearv(i); %dynamic viscosity

v(skip)=gamma(i)/rho(i); %kinematic viscosity

if T_Start(i)>=T_Surface
    
Ra(skip)= (g*alpha(i)*(T_Start(i)-T_Surface)*h(i)^3)/(td(i)*v(i)*1e9); %Rayleigh

elseif T_Start(i)<T_Surface
    
Ra(skip)= (g*alpha(i)*(T_Surface-T_Start(i))*h(i)^3)/(td(i)*v(i)*1e9); %Rayleigh
end



Pr(skip)=(v(i))/td(i);    %Prandtl Number
...
ylabel('Depth(km)')
xline(1708,'--','Critical Rayleigh Threshold')
figure
plot(Pr,h)
