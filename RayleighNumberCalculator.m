...
Ra(skip)= (rho(i)*g*alpha(i)*(T_Start(i)-T_Surface)*h(i)^3)/(td(i)*gamma(i)*1e9); %Rayleigh

elseif T_Start(i)<T_Surface
    
Ra(skip)= (rho(i)*g*alpha(i)*(T_Surface-T_Start(i))*h(i)^3)/(td(i)*gamma(i)*1e9); %Rayleigh
end

%PN(skip)=(Cp(i)*gamma(i))/k(i);    %Prandtl Number
